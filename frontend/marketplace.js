    ...
    try {
      ...
      async function searchProperties(searchParams = {}, pagination = {}){
        ...
        if (searchParams.maxPrice && searchParams.maxPrice > 0) {
            queryConstraints.push(where('rent', '<=', parseInt(searchParams.maxPrice)));
        }
        
        // Bedrooms filter
        if (searchParams.bedrooms && searchParams.bedrooms > 0) {
            queryConstraints.push(where('bedrooms', '==', parseInt(searchParams.bedrooms)));
        }
        
        // Bathrooms filter
        if (searchParams.bathrooms && searchParams.bathrooms > 0) {
            queryConstraints.push(where('bathrooms', '==', parseInt(searchParams.bathrooms)));
        }
        ...
        // Landlord filter (for landlord-specific searches)
        if (searchParams.landlordId) {
            queryConstraints.push(where('landlordId', '==', searchParams.landlordId));
        }
        
        // Availability filter
        if (searchParams.available !== undefined) {
            queryConstraints.push(where('available', '==', searchParams.available));
        }
        
        // Add ordering
        if (searchConfig.sortField) {
            queryConstraints.push(orderBy(searchConfig.sortField, searchConfig.sortDirection));
        }
        
        // Add pagination
        if (pagination.lastDocument) {
            queryConstraints.push(startAfter(pagination.lastDocument));
        }
        
        // Limit results
        queryConstraints.push(limit(searchConfig.maxResults));
        
        // Execute the query
        const propertiesQuery = query(collection(db, 'properties'), ...queryConstraints);
        ...
        properties = applyClientSideFilters(properties, searchParams);
        
        const results = {
            properties: properties,
            count: properties.length,
            hasMore: querySnapshot.docs.length === searchConfig.maxResults,
 ...
 */
function applyClientSideFilters(properties, searchParams) {
    ...
    if (searchParams.keyword && searchParams.keyword.trim()) {
        ...
        filteredProperties = filteredProperties.filter(property => {
            ...
            const description = (property.description || property.details || '').toLowerCase();
    ...
    if (searchParams.amenities && searchParams.amenities.length > 0) {
        filteredProperties = filteredProperties.filter(property => {
            ...
            return searchParams.amenities.every(requiredAmenity => 
...
function getPropertyAmenities(property) {
    ...
    } else if (property.amenities && typeof property.amenities === 'string') {
        amenities.push(...property.amenities.split(',').map(a => a.trim()));
    }
 ...
 */
async function quickSearch(searchTerm, filters = {}) {
    const searchParams = {
        keyword: searchTerm,
        available: true, // Only show available properties by default
        ...filters
    };
    
    return await searchProperties(searchParams);
}
 ...
 */
async function advancedSearch(formData) {
    const searchParams = {
        ...
        amenities: formData.getAll('amenities'), // For checkboxes
        available: formData.get('available') !== 'false'
    };
    
    // Remove empty values
    Object.keys(searchParams).forEach(key => {
        if (!searchParams[key] || searchParams[key] === '' || searchParams[key] === 'all') {
...
function displaySearchResults(results) {
    ...
    if (resultsCount) {
        resultsCount.textContent = `${results.count} propert${results.count !== 1 ? 'ies' : 'y'} found`;
    }
    ...
    const propertiesHTML = results.properties.map(property => `
        <div class="property-card" onclick="viewProperty('${property.id}')">
            <div class="property-image">
                <img src="${getPropertyImage(property)}" 
                     alt="${property.title || property.name}"
                     onerror="this.src='https://images.unsplash.com/photo-1564013799919-ab600027ffc6?ixlib=rb-1.2.1&auto=format&fit=crop&w=400&q=80'">
                ${property.featured ? '<div class="featured-badge">Featured</div>' : ''}
            </div>
            <div class="property-info">
                <h3 class="property-title">${property.title || property.name}</h3>
                <p class="property-location">
                    <i class="fas fa-map-marker-alt"></i>
                    ${property.location || property.address}
                </p>
                <div class="property-features">
                    <span><i class="fas fa-bed"></i> ${property.bedrooms || property.beds || property.rooms || 1}</span>
                    <span><i class="fas fa-bath"></i> ${property.bathrooms || property.baths || 1}</span>
                    <span><i class="fas fa-home"></i> ${property.propertyType || property.type || 'Property'}</span>
                </div>
                <div class="property-price">P${formatPrice(property.rent || property.price)}/month</div>
                ${property.deposit ? `<div class="property-deposit">Deposit: P${formatPrice(property.deposit)}</div>` : ''}
            </div>
        </div>
    `).join('');
    ...
    if (results.hasMore) {
        ...
        loadMoreBtn.onclick = () => loadMoreResults(results);
...
function getPropertyImage(property) {
    if (property.images && Array.isArray(property.images) && property.images[0]) {
        return property.images[0];
    }
    if (property.imageUrl) return property.imageUrl;
    if (property.image) return property.image;
    if (property.photos && Array.isArray(property.photos) && property.photos[0]) {
        return property.photos[0];
    }
    return 'https://images.unsplash.com/photo-1564013799919-ab600027ffc6?ixlib=rb-1.2.1&auto=format&fit=crop&w=400&q=80';
}

/**
 * Load more search results
 */
async function loadMoreResults(previousResults) {
    try {
        const moreResults = await searchProperties(
            previousResults.searchParams,
            { lastDocument: previousResults.lastDocument }
        );
        
        // Append new results to existing ones
        const existingContainer = document.getElementById('searchResults');
        const loadMoreBtn = existingContainer.querySelector('.load-more-btn');
        ...
        // Add new properties
        moreResults.properties.forEach(property => {
            const propertyCard = createPropertyCard(property);
            existingContainer.appendChild(propertyCard);
        });
        
        // Add new load more button if needed
        if (moreResults.hasMore) {
            const newLoadMoreBtn = document.createElement('button');
            newLoadMoreBtn.className = 'btn btn-secondary load-more-btn';
            newLoadMoreBtn.textContent = 'Load More Properties';
            newLoadMoreBtn.onclick = () => loadMoreResults(moreResults);
            existingContainer.appendChild(newLoadMoreBtn);
        }
        
    } catch (error) {
        console.error('Error loading more results:', error);
        showError('Failed to load more properties');
    }
}
...
function clearSearch() {
    ...
    // Optionally reload all properties
    // loadAllProperties();
}
